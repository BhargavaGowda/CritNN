...
data1 = np.loadtxt("data\ANN_RecombFullModular_BW.txt")
...
plt.plot(range(data1.shape[1]),mean,color='blue',label='Recombinant GA')
plt.fill_between(range(data1.shape[1]),mean+variance,mean-variance, color='blue',alpha=0.2)


data2 = np.loadtxt("data\ANN_OpenES_BW.txt")
...
plt.plot(range(data2.shape[1]),mean,color="orange",label='OpenES')
